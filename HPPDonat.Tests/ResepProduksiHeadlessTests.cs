...
public sealed class ResepProduksiHeadlessTests
{
    ...
    private sealed class FakeResepAppStateService : Services.IAppStateService
    {
        ...
        public Task UbahNamaVarianResepAsync(Models.ResepVarianModel? varian, string? namaBaru)
        {
            if (varian is null)
            {
                return Task.CompletedTask;
            ...
            if (target is not null && !string.IsNullOrWhiteSpace(namaBaru))
            {
                target.NamaVarian = namaBaru.Trim();
            }
